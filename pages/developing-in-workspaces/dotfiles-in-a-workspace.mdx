...
You can use the `--dotfiles` flag when creating a workspace, so that Devsy will
automatically clone and install your dotfiles in the workspace.

If you only specify the dotfiles repo, Devsy will clone your selected dotfiles
...
```
devsy up https://github.com/example/repo --dotfiles https://github.com/my-user/my-dotfiles-repo
```

Specifying a custom install script:

```
devsy up https://github.com/example/repo --dotfiles https://github.com/my-user/my-dotfiles-repo --dotfiles-script custom/location/install.sh
```
...
```
devsy context set-options -o DOTFILES_URL=https://github.com/89luca89/dotfiles -o DOTFILES_SCRIPT=bin/.local/bin/dotfiles
```
