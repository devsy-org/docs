...
```
devsy context set-options default -o SSH_INJECT_GIT_CREDENTIALS=false
```
...
If you don't want Devsy to inject the credentials, you can disable that via the following command for all workspaces:
```
devsy context set-options default -o SSH_INJECT_DOCKER_CREDENTIALS=false
```
...
```
devsy context set-options default -o GPG_AGENT_FORWARDING=true
```

Or when creating a workspace using:

```
devsy up --gpg-agent-forwarding my-workspace
```
