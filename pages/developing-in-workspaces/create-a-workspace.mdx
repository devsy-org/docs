...
:::info Add Provider
If you haven't configured a provider yet, Devsy will automatically open the provider modal for you. You can later add providers in the same way by navigating to 'Providers' > 'Add'
:::
...
- **Prebuild Repository**: A docker image repository such as `ghcr.io/my-org/my-repo` that contains prebuilds for this workspace. Devsy will try to automatically find a fitting prebuild for this workspace in the given repository.

Then press `Create Workspace` to launch the workspace.

:::info
Under the hood, the Desktop Application will call the CLI command `devsy up REPOSITORY`
:::

:::info Note
You can set the location of your devsy home by passing the `--devsy-home={home_path}` flag,
...
For example: setting `devsy-home=/mnt/c/Users/MyUser/` will result in a workspace path of something like `/mnt/c/Users/MyUser/.devsy/contexts/default/workspaces/...`
:::
...
devsy up github.com/microsoft/vscode-remote-try-node
```
...
```
Branch: devsy up github.com/microsoft/vscode-remote-try-node@main
Commit: devsy up github.com/microsoft/vscode-remote-try-node@sha256:15ba80171af11374143288fd3d54898860107323
PR:     devsy up github.com/microsoft/vscode-remote-try-node@pull/108/head # Only works for GitHub!
```
...
devsy up ghcr.io/my-org/my-repo:latest
```
...
```
devsy up my-workspace --source container:$CONTAINER_ID
```
...
```
devsy up my-workspace --recreate
```
