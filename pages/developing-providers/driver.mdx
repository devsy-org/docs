...
Devsy also allows building an image through Kubernetes with [buildkit](https://github.com/moby/buildkit).
Devsy will automatically determine if building is necessary or if a prebuild can be used. However, the `buildRepository` option needs to be specified for this to work.
...
- **buildRepository**: If defined, Devsy will build and push images to the given repository. If empty, Devsy will not build any images. Make sure you have push permissions for the given repository locally.
- **helperImage**: The image Devsy will use to find out the cluster architecture. Defaults to alpine.
...
agent:
  ...
  kubernetes:
    ...
    # namespace: my-namespace-for-devsy
    # context: default
    # clusterRole: ""
    # serviceAccount: ""
