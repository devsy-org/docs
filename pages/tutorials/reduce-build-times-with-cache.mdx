...
```
devsy context set-options -o REGISTRY_CACHE={registry}
```
...
```
devsy build my-workspace
```

Now the next time you or anyone in your team needs to build an image, it can pull from the cache. If you've made changes to your environment, then Devsy will detect these changes and are
built on top of the existing image layers.


#### How does Devsy detect changes?

Devsy parses your Dockerfile and .devcontainer.json to detect file paths that can affect your build context. Devsy traverses these files and makes a hash of the file contents to use as the
